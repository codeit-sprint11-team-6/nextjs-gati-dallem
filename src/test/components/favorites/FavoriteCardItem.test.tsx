...
jest.mock("@/components/common/MessageModal", () => ({
  __esModule: true,
...
describe("<FavoriteCardItem />", () => {
  ...
  test("모집 가능 상태: '참여하기' 버튼이 보이고 클릭 시 joinMutate 호출 & onSuccess에서 overlay 호출", () => {
    const favoriteGathering = {
      ...
      capacity: 5,
      participantCount: 2,
      registrationEnd: "2099-02-01T00:00:00Z",
    };

    renderWithQueryClient(<FavoriteCardItem {...favoriteGathering} />);
  ...
  test("모임 기본 정보가 Card.Title과 위치에 반영된다", () => {
    ...
    expect(screen.getByText("타입스크립트 북클럽")).toBeInTheDocument();
